...
function generatePlan(calories: number, country: string, budget: boolean, haveRaw: string) {
  ...
  // Prefer high protein, low carbs as defaults
          ...
          <div className="rounded-2xl border bg-white p-4">
            ...
            <ul className="list-disc list-inside">
              {result.grocery.map((g,i)=> (
                <li key={i}>{g.item} ×{g.count}</li>
              ))}
            </ul>
            <div className="mt-4 text-sm">
              Estimated total (3 days): <strong>${result.estCostTotal.toFixed(2)}</strong>
            </div>
          </div>
        </div>
      ) : (
        <div className="rounded-2xl border-2 border-dashed bg-white p-10 text-center text-slate-600">
          Use the filters above and click <span className="font-semibold">Generate 3-Day Plan</span> to preview.
        </div>
      )}
    </div>
  );
