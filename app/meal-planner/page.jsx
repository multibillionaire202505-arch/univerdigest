"use client";

import { useState } from "react";

type Recipe = {
  id: string;
  title: string;
  country: string;
  calories: number;
  protein: number;
  carbs: number;
  costPerServe: number;
  ingredients: string[];
  steps: string[];
};

const RECIPES: Recipe[] = [
  { id: "nig-chicken-stew", title: "Nigerian Tomato Chicken Stew", country: "Nigeria", calories: 520, protein: 45, carbs: 18, costPerServe: 2.3, ingredients: ["chicken thighs","tomato","onion","garlic","oil","bouillon"], steps:["Sear chicken, remove.","Sauté onion/garlic, add tomato & simmer.","Return chicken to finish."] },
  { id: "mx-tuna-lettuce-wraps", title: "Tuna Lettuce Wraps (Chipotle Lime)", country: "Mexico", calories: 340, protein: 36, carbs: 9, costPerServe: 1.6, ingredients:["canned tuna","lettuce","mayo","chipotle","lime","onion"], steps:["Mix tuna + mayo + chipotle + lime.","Spoon into lettuce leaves."] },
  { id: "it-turkey-zucchini", title: "Italian Turkey & Zucchini Skillet", country: "Italy", calories: 410, protein: 38, carbs: 12, costPerServe: 2.1, ingredients:["ground turkey","zucchini","bell pepper","garlic","tomato paste"], steps:["Brown turkey.","Add veg + paste; simmer 6–8 min."] },
  { id: "us-salmon-greenbeans", title: "Sheet-Pan Salmon & Green Beans", country: "USA", calories: 460, protein: 42, carbs: 10, costPerServe: 3.4, ingredients:["salmon","green beans","olive oil","lemon","salt","pepper"], steps:["Roast 400°F 12–14 min with oil, salt, pepper.","Finish with lemon."] },
  { id: "in-chana-salad", title: "Chana Masala Protein Salad (No Rice)", country: "India", calories: 380, protein: 21, carbs: 30, costPerServe: 1.2, ingredients:["chickpeas","tomato","onion","garam masala","cilantro","yogurt"], steps:["Toss warmed chickpeas with spices & veg.","Dollop yogurt."] },
];

const COUNTRIES = ["Any","USA","Mexico","Italy","Nigeria","India","Japan","Thailand","Greece","Brazil"];

function generatePlan(calories: number, country: string, budget: boolean, haveRaw: string) {
  let list = RECIPES.filter(r => country === "Any" || r.country === country);

  // Prefer high protein, low carbs as defaults for this MVP
  list = [...list].sort((a,b)=> b.protein - a.protein);
  list = [...list].sort((a,b)=> a.carbs - b.carbs);

  const have = haveRaw.split(",").map(s=>s.trim().toLowerCase()).filter(Boolean);
  if (have.length) {
    list = list
      .map(r => ({
        r,
        score: r.ingredients.reduce((acc, ing) => acc + (have.some(h => ing.toLowerCase().includes(h)) ? 1 : 0), 0)
      }))
      .sort((a,b)=> b.score - a.score)
      .map(x=>x.r);
  }

  if (budget) list = [...list].sort((a,b)=> a.costPerServe - b.costPerServe);

  // Pick recipes closest to target calories
  const byCal = [...list].sort((a,b)=> Math.abs(a.calories - calories) - Math.abs(b.calories - calories));
  const chosen = byCal.slice(0,3);

  const plan = [0,1,2].map(i => ({
    day: i+1,
    meals: [
      { type: "Breakfast", recipe: chosen[i % chosen.length] },
      { type: "Lunch", recipe: chosen[(i+1) % chosen.length] },
      { type: "Dinner", recipe: chosen[(i+2) % chosen.length] },
    ]
  }));

  // Build grocery list
  const groceryMap = new Map<string, number>();
  plan.forEach(d => d.meals.forEach(m => m.recipe.ingredients.forEach(ing => {
    const k = ing.toLowerCase();
    groceryMap.set(k, (groceryMap.get(k) || 0) + 1);
  })));
  const grocery = Array.from(groceryMap.entries()).map(([item,count]) => ({ item, count }));

  const estCostTotal = plan.reduce((sum,d) =>
    sum + d.meals.reduce((s,m)=> s + m.recipe.costPerServe, 0), 0);

  return { plan, grocery, estCostTotal };
}

export default function MealPlannerPage() {
  const [calories, setCalories] = useState("500");
  const [country, setCountry] = useState("Any");
  const [budget, setBudget] = useState(false);
  const [have, setHave] = useState("");
  const [result, setResult] = useState<ReturnType<typeof generatePlan> | null>(null);

  return (
    <div className="space-y-6">
      <header className="space-y-1">
        <h1 className="text-2xl md:text-3xl font-semibold tracking-tight">Meal Planner</h1>
        <p className="text-slate-600">Choose calories, country, and ingredients you already have. Get a 3-day plan + grocery list.</p>
      </header>

      <div className="rounded-2xl border bg-white p-4 space-y-4">
        <div className="grid gap-4 md:grid-cols-4">
          <div className="space-y-2">
            <label className="text-sm font-medium">Calories per meal</label>
            <input
              className="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-slate-400"
              inputMode="numeric"
              value={calories}
              onChange={(e)=> setCalories(e.target.value)}
              placeholder="e.g., 500"
            />
          </div>

          <div className="space-y-2">
            <label className="text-sm font-medium">Country / Cuisine</label>
            <select
              className="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-slate-400"
              value={country}
              onChange={(e)=> setCountry(e.target.value)}
            >
              {COUNTRIES.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
          </div>

          <div className="space-y-2">
            <label className="text-sm font-medium">Weekly budget (toggle)</label>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={budget} onChange={(e)=> setBudget(e.target.checked)} />
              Prioritize low cost
            </label>
          </div>

          <div className="space-y-2 md:col-span-1 md:row-span-2">
            <label className="text-sm font-medium">Ingredients you have</label>
            <textarea
              className="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-slate-400 min-h-[88px]"
              value={have}
              onChange={(e)=> setHave(e.target.value)}
              placeholder="chicken, onion, spinach"
            />
          </div>
        </div>

        <div className="flex gap-2">
          <button
            onClick={()=> setResult(generatePlan(Number(calories)||500, country, budget, have))}
            className="rounded-2xl px-4 py-2 bg-slate-900 text-white text-sm"
          >
            Generate 3-Day Plan
          </button>
          <button
            onClick={()=> setResult(null)}
            className="rounded-2xl px-4 py-2 bg-white text-slate-900 border border-slate-300 text-sm"
          >
            Reset
          </button>
        </div>
      </div>

      {result ? (
        <div className="grid gap-6 md:grid-cols-2">
          <div className="rounded-2xl border bg-white p-4">
            <div className="text-lg font-semibold mb-2">3-Day Meal Plan</div>
            <div className="space-y-4">
              {result.plan.map(day => (
                <div key={day.day} className="rounded-xl border border-slate-200 p-3">
                  <div className="font-semibold mb-2">Day {day.day}</div>
                  <ul className="space-y-2">
                    {day.meals.map((m, idx) => (
                      <li key={idx} className="flex items-start gap-2">
                        <span className="mt-1 text-xs font-medium text-slate-600 w-20 shrink-0">{m.type}</span>
                        <div>
                          <div className="font-medium">{m.recipe.title}</div>
                          <div className="text-xs text-slate-600">
                            {m.recipe.country} · {m.recipe.calories} kcal · {m.recipe.protein}g protein · {m.recipe.carbs}g carbs · ${m.recipe.costPerServe.toFixed(2)}/serving
                          </div>
                          <ol className="list-decimal ml-5 mt-1 text-xs text-slate-700 space-y-0.5">
                            {m.recipe.steps.map((s,i)=> <li key={i}>{s}</li>)}
                          </ol>
                        </div>
                      </li>
                    ))}
                  </ul>
                </div>
              ))}
            </div>
          </div>

          <div className="rounded-2xl border bg-white p-4">
            <div className="text-lg font-semibold mb-2">Smart Grocery List</div>
            <div className="text-sm text-slate-700 mb-3">Auto-consolidated across all meals. Tap to check off.</div>
            <ul className="grid grid-cols-2 md:grid-cols-3 gap-2">
              {result.grocery.map((g,i)=> (
                <li key={i} className="flex items-center gap-2 rounded-lg border border-slate-200 p-2">
                  <input type="checkbox" className="size-4 rounded" />
                  <span className="text-sm capitalize">{g.item}</span>
                  <span className="ml-auto text-xs text-slate-500">×{g.count}</span>
                </li>
              ))}
            </ul>
            <div className="mt-4 text-sm text-slate-700">
              Estimated total (3 days): <span className="font-semibold">${result.estCostTotal.toFixed(2)}</span>
            </div>
          </div>
        </div>
      ) : (
        <div className="rounded-2xl border-2 border-dashed bg-white p-10 text-center text-slate-600">
          Use the filters above and click <span className="font-semibold">Generate 3-Day Plan</span> to preview the flow.
        </div>
      )}
    </div>
  );
}
